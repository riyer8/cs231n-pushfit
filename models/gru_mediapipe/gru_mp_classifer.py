...
PLOT_DIR = "results/gru_mediapipe"
...
class PoseGRUClassifier(nn.Module):
    def __init__(self):
        super().__init__()
        self.gru = nn.GRU(input_size=NUM_KEYPOINTS * KEYPOINT_DIM, hidden_size=128, batch_first=True)
...
def main():
    ...
    final_model = PoseGRUClassifier()
    final_model, _, _ = train_model(final_model, full_loader, val_loader=None, epochs=epochs, suffix="_final")

    torch.save(final_model.state_dict(), "pose_gru_model_final.pt")
    print("✅ Final GRU model saved as pose_gru_model_final.pt")
