...
SUMMARY_PATH =  "crossval_summary.txt"
...
def accuracy_plot(train_accuracies, val_accuracies):
    plt.figure()
...
def main():
    ...
    accuracy_plot(train_accuracies, val_accuracies)

    summary_path = os.path.join(RESULTS_DIR, SUMMARY_PATH)
    with open(summary_path, "w") as f:
        for i in range(N_SPLITS):
            f.write(f"Fold {i + 1} Training Accuracy: {train_accuracies[i]:.4f}\n")
            f.write(f"Fold {i + 1} Val Accuracy:      {val_accuracies[i]:.4f}\n\n")
        f.write(f"Avg Training Accuracy:  {avg_train:.4f}\n")
        f.write(f"Avg Val Accuracy:       {avg_val:.4f}\n")
